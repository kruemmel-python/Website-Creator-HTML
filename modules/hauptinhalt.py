...
def open_hauptinhalt_gui(content_bg_var, content_text_var, content_font_family_var, content_font_size_var, content_text_align_var):
    """Erstellt eine GUI für den Hauptinhalt mit HTML-Darstellung und anpassbaren Stilen."""
    window = tk.Toplevel()
    window.title("Hauptinhalt Konfiguration")
    ...
    bg_color_var = tk.StringVar(value=content_bg_var.get())
    ...
    text_color_var = tk.StringVar(value=content_text_var.get())
    ...
    font_size_var = tk.StringVar(value=content_font_size_var.get())
    ...
    font_family_var = tk.StringVar(value=content_font_family_var.get())
    ...
    text_align_var = tk.StringVar(value=content_text_align_var.get())
    ...
    # Eingabebox für Hauptinhalt-HTML-Code
    content_code_label = ttk.Label(right_frame, text="Hauptinhalt-Code:")
    content_code_label.pack(anchor="nw")
    content_code_box = tk.Text(right_frame, height=10)
    content_code_box.insert("1.0", "<p>Dies ist der Hauptinhalt der Anwendung.</p>")
    content_code_box.pack(fill="x", padx=5, pady=5)
    ...
    def open_webview():
        ...
        content_code = content_code_box.get("1.0", tk.END).strip()
        css = f"""
        <style>
            .content {{
        ...
        """
        styled_html = f"<html><head>{css}</head><body><div class='content'>{content_code}</div></body></html>"
        webview.create_window('Live-Ansicht Hauptinhalt', html=styled_html, width=600, height=400)
        webview.start()

    def apply_changes():
        """Übernimmt die aktuellen Einstellungen in die Haupt-GUI."""
        content_bg_var.set(bg_color_var.get())
        content_text_var.set(text_color_var.get())
        content_font_family_var.set(font_family_var.get())
        content_font_size_var.set(font_size_var.get())
        content_text_align_var.set(text_align_var.get())
